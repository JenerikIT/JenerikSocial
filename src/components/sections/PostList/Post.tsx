...
const Post = memo(
  ({
    ...
    const [addPostFavourite] = useAddPostFavouriteMutation();
    createdAt = createdAt.slice(0, 10);
    const [deletePost] = useDeletePostMutation();
    const { data: AuthData } = useAuthMeQuery();
    const isAuthor = useMemo(() => {
      return AuthData?.userData?._id === user._id;
    }, [AuthData, user._id]);
    const { data: AllMyLikedPost, isLoading } = useGetAllLikedPostsQuery();
    const isFavourite = useMemo(() => {
      if (isLoading) return;
      return AllMyLikedPost?.items.some((obj) => obj._id === _id);
    }, [isLoading, AllMyLikedPost?.items]);
    const isReallyLiked = isFavourite;
    console.log(isReallyLiked);
    const dispatch = useAppDispatch();
    const onClickAddFavourite = async () => {
      try {
        if (isReallyLiked && likeCount !== 0) {
          await addPostFavourite(_id);
          dispatch(removeFavouritePost({ _id }));
          console.log("пост вроде удален");
        } else {
          const itemsPost = {
            _id,
            text,
          ...
          await addPostFavourite(_id);
          dispatch(addFavourite(itemsPost));
        }
      } catch (error) {
        console.log(error);
      }
          ...
          <div className="post__footer">
            <div className="tags">
              {!tags.length && isAuthor ? (
                ...
                tags.map((name, i) => <HashTag title={name} key={i} />)
              )}
            </div>
            <div className="views">
          ...
          <div className={`modal-Post ${complain ? "active" : ""}`}>
            ...
            <div className="content">
              ...
              <div className="content-info">
                <div className="content-info__description">{text}</div>
